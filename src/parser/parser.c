...
static ast_node_t* parse_parameter(void);
static ast_node_t* parse_variable_decl_after_name(
    type_t* var_type,
    char* name,
    const char* semicolon_msg);
static token_t* parser_current(void);
static bool parser_match(token_type_t type);
static ast_node_t* ast_node_create(ast_node_type_t type);
static ast_node_t* parse_function_after_name(char* name, type_t* return_type);
...
static void parser_error(const char* msg, token_t* tok) {
    parser_report_error(msg, tok);
    parser->error_count++;
}

static void parser_error_current(const char* msg) {
    parser_error(msg, parser_current());
}
...
static int8_t parse_sign_state(sign_state_t* sign_state) {
    for (uint8_t i = 0; i < (uint8_t)(sizeof(k_sign_tokens) / sizeof(k_sign_tokens[0])); i++) {
        if (parser_match(k_sign_tokens[i].tok)) {
            if (*sign_state != SIGN_NONE && *sign_state != k_sign_tokens[i].sign) {
                parser_error_current(ERR_SIGNED_UNSIGNED);
                return -1;
            }
...
static token_type_t parser_peek_type(void) {
...
static bool parser_check(token_type_t type) {
...
static bool parser_consume(token_type_t type, const char* msg) {
    if (parser_check(type)) {
        parser_advance();
        return true;
    }

    parser_error_current(msg);
    return false;
}
...
static bool parser_consume_expected(
    ...
    parser_report_expected(type, msg, parser_current());
    parser->error_count++;
    return false;
}
...
static type_t* parse_type(void) {
    ...
    if (type->kind == TYPE_VOID) {
        if (sign_state != SIGN_NONE) {
            parser_error_current(ERR_VOID_SIGN);
            type_destroy(type);
            return NULL;
        }
        return type;
...
static int8_t parse_array_suffix(uint16_t* out_len, bool allow_unsized) {
    if (!out_len) return -1;
    if (!parser_match(TOK_LBRACKET)) return 0;
    if (parser_check(TOK_RBRACKET)) {
        if (!allow_unsized) {
            parser_error_current(ERR_EXPECT_ARRAY_LEN);
            return -1;
        }
        parser_advance();
        *out_len = 0;
    ...
    if (!parser_check(TOK_NUMBER)) {
        parser_error_current(ERR_EXPECT_ARRAY_LEN);
        return -1;
    }
    token_t* len_tok = parser_current();
    int16_t len = len_tok->int_val;
    parser_advance();
    if (len <= 0) {
        parser_error(ERR_ARRAY_POS, len_tok);
        return -1;
    }
    if (!parser_consume_expected(TOK_RBRACKET, ERR_AFTER_ARRAY_LEN)) {
        return -1;
    }
...
static type_t* parse_array_type(
    ...
    if (parser_check(TOK_LBRACKET)) {
        parser_error_current("Only single-dimension arrays supported");
        type_destroy(base);
        return NULL;
    }
    if (base->kind == TYPE_VOID) {
        parser_error_current(ERR_ARRAY_VOID);
        type_destroy(base);
        return NULL;
    }
    if (as_pointer) {
...
static ast_node_t* parse_variable_decl_after_name(
    ...
    int8_t array_suffix = parse_array_suffix(&array_len, false);
    if (array_suffix < 0) {
        type_destroy(var_type);
        cc_free(name);
        return NULL;
    }
    var_type = parse_array_type(var_type, array_len, array_suffix, false);
    if (!var_type) {
        cc_free(name);
        return NULL;
    }
    ...
    parser_consume_expected(TOK_SEMICOLON, semicolon_msg);
    return node;
}

static ast_node_t* parse_primary(void) {
    token_t* tok = parser_current();
    ...
    if (tok->type == TOK_IDENTIFIER) {
        ...
        if (parser_check(TOK_LPAREN)) {
            ...
            if (!parser_check(TOK_RPAREN)) {
                ...
                do {
                    ...
                    if (parser_check(TOK_COMMA)) {
                        parser_advance();
                    } else {
                        break;
                    }
                } while (!parser_check(TOK_RPAREN) && !parser_check(TOK_EOF));
            ...
            if (!parser_consume_expected(TOK_RPAREN, NULL)) {
                ast_node_destroy(call);
                return NULL;
            }
        ...
        base = node;
    } else if (parser_match(TOK_LPAREN)) {
        ast_node_t* expr = parse_expression();
        parser_consume_expected(TOK_RPAREN, NULL);
        base = expr;
    } else {
        parser_error_current(ERR_UNEXPECTED_EXPR);
        parser_advance();
        return NULL;
    }

    while (base && parser_match(TOK_LBRACKET)) {
        ast_node_t* index = parse_expression();
        if (!index) {
            ast_node_destroy(base);
            return NULL;
        }
        if (!parser_consume_expected(TOK_RBRACKET, ERR_AFTER_INDEX)) {
            ast_node_destroy(base);
    ...
    while (base && (parser_check(TOK_PLUS_PLUS) || parser_check(TOK_MINUS_MINUS))) {
        token_type_t op = parser_current()->type;
        parser_advance();
 ...
 * expression: term
 */
static ast_node_t* parse_unary(void);
static ast_node_t* parse_factor(void);
static ast_node_t* parse_term(void);
static ast_node_t* parse_shift(void);
static ast_node_t* parse_comparison(void);
static ast_node_t* parse_bitwise_and(void);
static ast_node_t* parse_bitwise_xor(void);
static ast_node_t* parse_bitwise_or(void);
static ast_node_t* parse_logical_and(void);
static ast_node_t* parse_logical_or(void);
...
static ast_node_t* parse_binary_left(ast_node_t* (*next)(void),
                                     const binop_map_t* ops,
                                     uint8_t op_count) {
    ast_node_t* left = next();
    if (!left) return NULL;

    while (1) {
        token_type_t tok = parser_current()->type;
        bool matched = false;
...
static ast_node_t* parse_unary(void) {
    ...
    }
    {
        ...
        for (uint8_t i = 0; i < (uint8_t)(sizeof(unary_ops) / sizeof(unary_ops[0])); i++) {
            if (parser_match(unary_ops[i].token)) {
                ast_node_t* operand = parse_unary();
...
static ast_node_t* parse_term(void) {
    ...
    return parse_binary_left(parse_factor, ops,
                             (uint8_t)(sizeof(ops) / sizeof(ops[0])));
}
...
static ast_node_t* parse_shift(void) {
    ...
    return parse_binary_left(parse_term, ops,
                             (uint8_t)(sizeof(ops) / sizeof(ops[0])));
}
...
static ast_node_t* parse_comparison(void) {
    ...
    return parse_binary_left(parse_shift, ops,
                             (uint8_t)(sizeof(ops) / sizeof(ops[0])));
}
...
static ast_node_t* parse_bitwise_and(void) {
    ...
    return parse_binary_left(parse_comparison, ops,
                             (uint8_t)(sizeof(ops) / sizeof(ops[0])));
}
...
static ast_node_t* parse_bitwise_xor(void) {
    ...
    return parse_binary_left(parse_bitwise_and, ops,
                             (uint8_t)(sizeof(ops) / sizeof(ops[0])));
}
...
static ast_node_t* parse_bitwise_or(void) {
    ...
    return parse_binary_left(parse_bitwise_xor, ops,
                             (uint8_t)(sizeof(ops) / sizeof(ops[0])));
}
...
static ast_node_t* parse_logical_and(void) {
    ...
    return parse_binary_left(parse_bitwise_or, ops,
                             (uint8_t)(sizeof(ops) / sizeof(ops[0])));
}
...
static ast_node_t* parse_logical_or(void) {
    ...
    return parse_binary_left(parse_logical_and, ops,
                             (uint8_t)(sizeof(ops) / sizeof(ops[0])));
}
...
static ast_node_t* parse_expression(void) {
    ...
    if (parser_check(TOK_ASSIGN)) {
        parser_advance();
        ast_node_t* right = parse_expression(); /* Recursive for right-associativity */
        if (!right) {
            ast_node_destroy(left);
            return NULL;
        }
...
static ast_node_t* parse_statement(void) {
    ...
    if (var_type) {
        while (parser_match(TOK_STAR)) {
            var_type = type_create_pointer(var_type);
        }

        token_t* name_tok = parser_current();
        char* name = name_tok->value;
        name_tok->value = NULL;
        if (!parser_consume(TOK_IDENTIFIER, ERR_EXPECT_IDENT)) {
            type_destroy(var_type);
            cc_free(name);
            return NULL;
        }
        return parse_variable_decl_after_name(
            var_type, name, ERR_AFTER_VAR_DECL);
    }

    if (parser_match(TOK_IF)) {
        ast_node_t* node = ast_node_create(AST_IF_STMT);
        if (!node) return NULL;

        if (!parser_consume_expected(TOK_LPAREN, ERR_AFTER_IF)) {
            cc_free(node);
            return NULL;
        }
        ...
        if (!parser_consume_expected(TOK_RPAREN, ERR_AFTER_IF)) {
            ast_node_destroy(node);
            return NULL;
        }
    ...
    if (parser_match(TOK_WHILE)) {
        ast_node_t* node = ast_node_create(AST_WHILE_STMT);
        if (!node) return NULL;

        if (!parser_consume_expected(TOK_LPAREN, ERR_AFTER_WHILE)) {
            cc_free(node);
            return NULL;
        }
        ...
        if (!parser_consume_expected(TOK_RPAREN, ERR_AFTER_WHILE)) {
            ast_node_destroy(node);
            return NULL;
        }
    ...
    if (parser_match(TOK_FOR)) {
        ast_node_t* node = ast_node_create(AST_FOR_STMT);
        if (!node) return NULL;

        if (!parser_consume_expected(TOK_LPAREN, ERR_AFTER_FOR)) {
            cc_free(node);
            return NULL;
        }

        /* Init expression (or declaration) */
        if (!parser_check(TOK_SEMICOLON)) {
        ...
        /* Condition */
        if (!parser_check(TOK_SEMICOLON)) {
            node->data.for_stmt.condition = parse_expression();
        } else {
            node->data.for_stmt.condition = NULL;
        }
        if (!parser_consume_expected(TOK_SEMICOLON, ERR_AFTER_FOR_COND)) {
            ast_node_destroy(node);
            return NULL;
        }
        ...
        /* Increment */
        if (!parser_check(TOK_RPAREN)) {
            node->data.for_stmt.increment = parse_expression();
        } else {
            node->data.for_stmt.increment = NULL;
        }

        if (!parser_consume_expected(TOK_RPAREN, ERR_AFTER_FOR)) {
            ast_node_destroy(node);
            return NULL;
        }
    ...
    if (parser_match(TOK_RETURN)) {
        ...
        parser_consume_expected(TOK_SEMICOLON, NULL);
        return node;
    }

    if (parser_match(TOK_BREAK)) {
        ast_node_t* node = ast_node_create(AST_BREAK_STMT);
        parser_consume_expected(TOK_SEMICOLON, NULL);
        return node;
    }

    if (parser_match(TOK_CONTINUE)) {
        ast_node_t* node = ast_node_create(AST_CONTINUE_STMT);
        parser_consume_expected(TOK_SEMICOLON, NULL);
        return node;
    }

    if (parser_match(TOK_GOTO)) {
        token_t* name_tok = parser_current();
        char* name = name_tok->value;
        name_tok->value = NULL;
        if (!parser_consume(TOK_IDENTIFIER, ERR_EXPECT_IDENT)) {
    ...
    if (parser_check(TOK_IDENTIFIER) && parser_peek_type() == TOK_COLON) {
        token_t* name_tok = parser_current();
        char* name = name_tok->value;
        name_tok->value = NULL;
        parser_advance();
        if (!parser_consume_expected(TOK_COLON, NULL)) {
            cc_free(name);
            return NULL;
        }
    ...
    if (parser_match(TOK_LBRACE)) {
        ...
        while (!parser_check(TOK_RBRACE) && !parser_check(TOK_EOF) &&
        ...
        parser_consume_expected(TOK_RBRACE, NULL);
        return node;
    }
    ...
    ast_node_t* expr = parse_expression();
    if (expr) {
        parser_consume_expected(TOK_SEMICOLON, ERR_AFTER_EXPR);
        return expr;
    }
...
static ast_node_t* parse_parameter(void) {
    ...
    type_t* var_type = parse_type();
    if (!var_type) {
        parser_error_current(ERR_EXPECT_PARAM_TYPE);
    ...
    char* name = name_tok->value;
    name_tok->value = NULL;
    if (!parser_consume(TOK_IDENTIFIER, ERR_EXPECT_PARAM_NAME)) {
        type_destroy(var_type);
        cc_free(name);
        ast_node_destroy(param);
        return NULL;
    ...
    uint16_t array_len = 0;
    int8_t array_suffix = parse_array_suffix(&array_len, true);
    if (array_suffix < 0) {
        type_destroy(var_type);
        cc_free(name);
        ast_node_destroy(param);
    ...
    var_type = parse_array_type(var_type, array_len, array_suffix, true);
    if (!var_type) {
        cc_free(name);
        ast_node_destroy(param);
        return NULL;
...
static ast_node_t* parse_declaration(void) {
    ...
    if (decl_type) {
        ...
        char* name = name_tok->value;
        name_tok->value = NULL;
        if (!parser_consume(TOK_IDENTIFIER, ERR_EXPECT_FUNC_OR_VAR)) {
        ...
        if (parser_check(TOK_LPAREN)) {
            return parse_function_after_name(name, decl_type);
        }
        return parse_variable_decl_after_name(
            decl_type, name, ERR_AFTER_GLOBAL_DECL);
    }

    token_t* tok = parser_current();
    parser_error(ERR_EXPECT_DECL, tok);
    return NULL;
}
...
static ast_node_t* parse_function_after_name(char* name, type_t* return_type) {
    ...
    if (!parser_consume_expected(TOK_LPAREN, NULL)) {
        ast_node_destroy(node);
        return NULL;
    }

    /* Parse parameter list */
    if (parser_check(TOK_VOID) && parser_peek_type() == TOK_RPAREN) {
        parser_advance(); /* consume 'void' in (void) */
    } else {
        ...
        while (!parser_check(TOK_RPAREN) && !parser_check(TOK_EOF)) {
            ...
            ast_node_t* param = parse_parameter();
            if (!param) {
                while (!parser_check(TOK_RPAREN) && !parser_check(TOK_EOF)) {
                    parser_advance();
                }
...
ast_node_t* parser_parse_next(void) {
    if (!parser || parser_check(TOK_EOF)) return NULL;
